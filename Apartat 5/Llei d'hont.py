...
cursor.execute(f"SELECT sum(vots) as vots from vots_candidatures_prov where provincia_id=(SELECT provincia_id FROM provincies WHERE lower(nom)='{x}')")
vots_totals= cursor.fetchone()
blancs = conexion.cursor()
blancs.execute(f"SELECT sum(vots_blanc) as VotsBlancs FROM eleccions_municipis em INNER JOIN municipis m on m.municipi_id=em.municipi_id WHERE m.provincia_id=(SELECT provincia_id FROM provincies WHERE lower(nom)='{x}')")
vots_blancs=blancs.fetchone()
nulls = conexion.cursor()
nulls.execute(f"SELECT sum(vots_nuls) as VotsNuls FROM eleccions_municipis em INNER JOIN municipis m on m.municipi_id=em.municipi_id WHERE m.provincia_id=(SELECT provincia_id FROM provincies WHERE lower(nom)='{x}')")
