...
try:
    with open(path, "r", encoding="utf-8") as fitxer:
        for linia in fitxer:
            if linia[16:18] == "99":
                municipi_id = cursor.execute(f"SELECT municipi_id FROM municipis WHERE (codi_ine = '{linia[13:16]}') AND (provincia_id = (SELECT provincia_id FROM provincies WHERE codi_ine = '{linia[11:13]}'))")
