...
def ImportarComunitatsAutonomes():
    ...
    with open(path, "r") as fitxer:
        for linia in fitxer:
            if linia[11:13] != "99":
                ...
                cursor.execute(insert,(comunitat_aut_id[0],nom,codi_ine,escons))
            
def ImportarMunicipis():
    path = "Documentació/02201904_MESA/05021904.DAT"
    insert = ("INSERT INTO municipis (codi_ine,nom,provincia_id,districte) VALUES (%s,%s,%s,%s)")
    with open(path, "r") as fitxer:
        for linia in fitxer:
...
def ImportarEleccionsMunicipis():
    path = "Documentació/02201904_MESA/05021904.DAT"
    ...
    with open(path, "r") as fitxer:
        for linia in fitxer:
            if linia[16:18] == "99":
                ...
                vots_emesos = int(linia[189:197]) + int(linia[197:205]) + int(linia[205:213])
                ...
                cursor.execute(insert(1,municipi_id[0],num_meses,cens,vots_emesos,vots_valids,vots_candidatures,vots_blanc,vots_nuls))

def ImportarCandidatures():
    ...
    with open(path, "r") as fitxer:
        for linia in fitxer:
            ...
            cursor.execute(insert,(1,codi,nomCurt,nomLlarg,codiProvincial,codiAutonomic,codiNacional))

def ImportarPersones():
    path = "Documentació/02201904_MESA/04021904.DAT"
    ...
    with open(path, "r") as fitxer:
        for linia in fitxer:
            if linia[9:11] != "99":
                ...
                tipus = linia[24:25]
                cursor.execute(insert,(candidatura_id[0],persona_id[0],provincia_id[0],num_ordre,tipus))
                
def ImportarVotsMunicipal():
    ...
    with open(path, "r") as fitxer:
        for linia in fitxer:
            if linia[11:13] != "99":
                ...
                cursor.execute(insert,(provincia_id[0],candidatura_id[0],vots,candidats_obtinguts))
                
def ImportarVotsAutonomic():
    path = "Documentació/02201904_MESA/08021904.DAT"
    ...
    with open(path, "r") as fitxer:
        for linia in fitxer:
            if linia[11:13] == "99" and linia[9:11] != "99":
                ...
                cursor.execute(insert,(comunitat_autonoma_id[0],candidatura_id[0],vots))
...
ImportarVotsProvincial()
ImportarVotsAutonomic()
